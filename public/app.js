...
// Пара цветов для обложки профиля — тот же принцип, что и avatarBg (стабильно
// по имени), но два разных цвета из палитры для градиента "тёмный → светлый",
// плюс второй хеш, чтобы цвета не совпадали слишком часто.
function coverColors(name) {
  const colors = ['#e17076', '#7bc862', '#65aadd', '#a695e7', '#ee7aae', '#6ec9cb', '#faa774', '#4f95d1', '#4fc9af', '#c087e0'];
  let h1 = 0;
  for (let i = 0; i < name.length; i++) h1 = name.charCodeAt(i) + ((h1 << 5) - h1);
  let h2 = 0;
  for (let i = 0; i < name.length; i++) h2 = name.charCodeAt(i) * 7 + ((h2 << 5) - h2) + 13;
  const c1 = colors[Math.abs(h1) % colors.length];
  let idx2 = Math.abs(h2) % colors.length;
  if (idx2 === Math.abs(h1) % colors.length) idx2 = (idx2 + 3) % colors.length;
  return [c1, colors[idx2]];
}
...
socket.on('profile:data', (profile) => {
  ...
  const [coverC1, coverC2] = coverColors(profile.name);
  el('profile-cover').style.setProperty('--profile-cover-c1', coverC1);
  el('profile-cover').style.setProperty('--profile-cover-c2', coverC2);
