...
// ------------------------------------------------------------------
// QR-код профиля (?user=NOVA-ID в URL) — сканируя QR из чужого
// профиля, попадаешь по ссылке вида /?user=NOVA-XXXXXX и сразу
// открываешь профиль этого человека, не вводя ничего руками.
// ------------------------------------------------------------------
let pendingProfileUserId = null;

function checkUserQrInUrl() {
  const params = new URLSearchParams(location.search);
  const uid = params.get('user');
  if (!uid) return;
  pendingProfileUserId = uid;
  history.replaceState({}, '', location.pathname);
  if (me) openPendingProfileFromUrl();
}

function openPendingProfileFromUrl() {
  if (!pendingProfileUserId) return;
  const uid = pendingProfileUserId;
  pendingProfileUserId = null;
  openProfile(uid);
}

el('profile-qr-btn').addEventListener('click', () => {
  if (!me) return;
  const url = `${location.origin}${location.pathname}?user=${encodeURIComponent(me.novaId)}`;
  const box = el('profile-qr-code');
  box.innerHTML = '';
  if (window.QRCode) {
    new QRCode(box, { text: url, width: 156, height: 156, colorDark: '#1c1e21', colorLight: '#ffffff' });
  } else {
    box.textContent = me.novaId;
  }
  el('profile-qr-novaid').textContent = me.novaId;
  el('profile-qr-overlay').classList.remove('hidden');
});
...
// Строка контактов вверху сайдбара: свой аккаунт первым, затем контакты
// (те, кто сейчас в сети — выше остальных), затем кнопка "Добавить",
// открывающая обычный экран контактов.
function renderContactsStrip() {
  const strip = el('contacts-strip');
  if (!strip || !me) return;
  strip.innerHTML = '';

  const selfItem = document.createElement('div');
  selfItem.className = 'contacts-strip-item';
  selfItem.innerHTML = `
    <div class="avatar" style="background:${avatarBg(me.name)}">${initials(me.name)}</div>
    <span class="contacts-strip-name">Вы</span>
  `;
  selfItem.addEventListener('click', () => openProfile(me.id));
  strip.appendChild(selfItem);

  const sorted = [...myContacts].sort((a, b) => (b.online ? 1 : 0) - (a.online ? 1 : 0));
  sorted.forEach((person) => {
    const item = document.createElement('div');
    item.className = 'contacts-strip-item';
    item.innerHTML = `
      <div class="contacts-strip-avatar-wrap">
        <div class="avatar" style="background:${avatarBg(person.name)}">${initials(person.name)}</div>
        ${person.online ? '<span class="online-dot"></span>' : ''}
      </div>
      <span class="contacts-strip-name">${escapeHtml(person.name)}</span>
    `;
  ...
  addItem.className = 'contacts-strip-item';
  addItem.innerHTML = `
    <div class="contacts-strip-add">+</div>
    <span class="contacts-strip-name">Добавить</span>
  `;
  addItem.addEventListener('click', () => el('open-contacts').click());
...
socket.on('profile:data', (profile) => {
  ...
  el('profile-qr-btn').classList.toggle('hidden', !profile.isSelf);
