...
//
// ВАЖНО, куда пишется файл: раньше он лежал ВНУТРИ папки проекта
// (рядом с server/). Из-за этого при каждой замене/перескачивании
// файлов проекта (новая папка, распаковка архива поверх старой)
// папка data/ либо не переносилась, либо стиралась вместе с кодом —
// и аккаунты "пропадали", хотя сам механизм сохранения работал
// правильно. Теперь по умолчанию файл хранится ВНЕ папки проекта —
// в домашней директории пользователя — и переживает любые манипуляции
// с папкой кода (замену файлов, git pull, распаковку нового zip и
// т.п.). Путь можно переопределить переменной окружения NOVA_DATA_DIR
// (пригодится для деплоя на хостинг с постоянным volume).
//
...
function loadState(state) {
  ensureDataDir();
  console.log(`[store] Файл данных: ${DATA_FILE}`);
